...
class ImportaPlanilha{
	 ...
	 */
	private function isRegistroDuplicado($codigo=null){
		$retorno = false;

		try{
			if(!empty($codigo)):
				$sql = 'SELECT codigo FROM bt_lista_principal WHERE codigo = ?';
	...
	public function insertDados(){
		...
		try{
			$sql = 'INSERT INTO tb_lista_principal (interprete, codigo, titulo, inicio, idioma)VALUES(?, ?, ?, ?, ?)';
			$stm = $this->conexao->prepare($sql);
